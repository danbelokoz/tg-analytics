...
# Пост-вакансия (а не реклама/новость). Только «сильные» маркеры найма: общие
# слова («условия», «remote», «нужен») убраны — они встречаются в новостях/историях
# и давали ложные срабатывания (напр. заметка «…оказался не сам рецепт, а условия»).
_VAC = re.compile(
    r"(ваканс|ищ(?:ем|ет|у|ете|ут)\b|в поиске\s+\w|требуется|требуются|в команду|hiring|"
    r"we[' ]?re looking|we are looking|looking for a|открыт[аы]?\s+(?:вакансия|позиц|наб)|"
    r"обязанност|требовани[яей]|мы предлагаем|что предлагаем|условия работы|откликнут|"
    r"присылайт\w*\s+резюме|резюме\s+(?:на|отправ|направ|присыл|ждем|ждём|жду)|"
    r"apply now|join (?:our|the) team|должность[:\s]|позиция[:\s])", re.I)
# NB: «з/п», «зарплат», «оклад», «резюме» (без контекста) убраны из «сильных» маркеров —
# они встречаются в контентных постах-советах про зарплату/резюме («Никогда не ставьте
# зарплату в резюме»), и давали ложные срабатывания. Зарплата всё равно ловится как
# «З/п указана» (_SALARY) и учитывается как corroboration к роли, а не сама по себе.
...
# Реклама / курсы / промо / КОНТЕНТ (советы, гайды, «как…», подборки, listicle-посты) —
# упоминают роли/зарплату/резюме, но это не вакансии. Отсекаем такие посты, если в них
# нет явных маркеров найма (_VAC). Осторожно: «курс» ловим по границе слова.
_ADV = re.compile(
    r"\bкурс|обучени|научит[ьс]|запиш[иу]|вебинар|интенсив|мастер-?класс|марафон|"
    r"буткемп|bootcamp|наставник|подписывайт|подписка на|промокод|розыгрыш|скидк|"
    r"\bреклам|\berid\b|разбор.{0,15}резюме|карьерн\w*\s+консультац|"
    r"никогда не |как\s+(?:напис|состав|пройти|устро|получ|найти|выбра|стать|повыс|оформ|заполн|проход|отвеча|подготов)|"
    r"\d+\s*(?:ошиб|совет|способ|причин|правил|лайфхак|признак|фраз)|"
    r"\bсоветы|лайфхак|чек-?лист|памятк|что делать,?\s+если|"
    r"почему\s+(?:вы|вам|тебе|не стоит|нельзя|важно|нужно)|"
    r"подборк\w*\s+(?:книг|материал|канал|курс|ресурс|инструмент|подкаст|стат)", re.I)
# СТРУКТУРНЫЕ маркеры вакансии: глаголы найма, разделы («обязанности/требования»),
# CTA («откликнуться/присылайте резюме»), «#вакансия» / «Вакансия:». В ОТЛИЧИЕ от «ваканс»
# как слова — их нет в контент-постах, которые лишь УПОМИНАЮТ вакансии («к каждой
# вакансии есть вилка»). Контент-пост (_ADV) без структурного маркера → не вакансия.
_STRUCT_VAC = re.compile(
    r"ищ(?:ем|ет|у|ете|ут)\b|требуется|требуются|в команду|hiring|we[' ]?re looking|"
    r"we are looking|looking for a|открыт[аы]?\s+(?:вакансия|позиц|наб)|обязанност|"
    r"требовани[яей]|мы предлагаем|что предлагаем|условия работы|откликнут|"
    r"присылайт\w*\s+резюме|резюме\s+(?:на|отправ|направ|присыл|ждем|ждём|жду)|"
    r"apply now|join (?:our|the) team|должность[:\s]|позиция[:\s]|#вакансия|"
    r"ваканси[яию]\s*[:\-—]", re.I)
# Навигация/оглавление канала («Сотни вакансий и полезных материалов», «Свежие
# вакансии по городам: Работа в Москве / Санкт-Петербурге / …»). Содержит слово
# «вакансии», но это меню канала, а не вакансия — отсекаем ДО всех прочих правил
# (даже до _VAC/_DIGEST): по маркеру-фразе (_NAV) либо по списку из 3+ ссылок
# «Работа в <Город>» (_CITY).
_NAV = re.compile(
    r"сотни\s+ваканс|тысяч[аи]?\s+ваканс|ваканси[ий]\s+по\s+город|свежие\s+ваканси|"
    r"полезны[йх]\s+материал|навигаци[яю]\s+по\s+канал|интересные\s+стать", re.I)
_CITY = re.compile(r"работа\s+в[оo]?\s+[А-ЯЁ]", re.I)
...
def tag_post(text):
    if _NAV.search(text) or len(_CITY.findall(text)) >= 3:
        return [], False   # навигация/оглавление канала — не вакансия
    ...
    # Реклама/курсы/промо/контент (советы, «как…», подборки) без СТРУКТУРНОГО маркера
    # найма — не вакансия, даже если в тексте мелькает слово «вакансия» или есть роль.
    if _ADV.search(text) and not _STRUCT_VAC.search(text):
        is_vacancy = False
...
async def run(usernames):
    ...
    async with aiohttp.ClientSession(timeout=timeout, headers={"User-Agent": UA}) as session:
        ...
        for i, coro in enumerate(asyncio.as_completed(tasks), 1):
            ...
            if err:
                failed.append(f"@{username}: {str(err)[:80]}")
...
def main():
    ...
    vac, failed = asyncio.run(run(usernames))
    # Статус прогона → админ-дешборд (total = число собранных вакансий из постов)
    status.record("tg", total=vac, failed=len(failed), prev=status.prev_total("tg"),
                  meta={"channels": len(usernames), "failed": failed[:50]})
