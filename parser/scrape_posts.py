...
_VAC = re.compile(
    ...
    r"обязанност|требовани[яей]|мы предлагаем|что предлагаем|условия работы|"
    r"з/п|зарплат|оклад|вилка\s|резюме|откликнут|apply now|join (?:our|the) team|"
    r"должность[:\s]|позиция[:\s])", re.I)
...
# Реклама / курсы / промо / контент — упоминают роли, но это не вакансии. Отсекаем такие
# посты, если в них нет явных маркеров найма (_VAC): «аналитик» в посте про курсы
# аналитики ≠ вакансия. Осторожно: «курс» ловим по границе слова (не «конкурс»/«дискурс»).
_ADV = re.compile(
    ...
    r"\bреклам|\berid\b|разбор.{0,15}резюме|карьерн\w*\s+консультац", re.I)
